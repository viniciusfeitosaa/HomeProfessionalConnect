    ...
    app.post('/api/payment/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
        ...
        switch (event.type) {
            case 'payment_intent.succeeded':
                ...
                await storage.updatePaymentReferenceStatus(paymentIntentSucceeded.id, 'approved', 'succeeded', paymentIntentSucceeded.id, new Date());
            ...
            case 'payment_intent.payment_failed':
                ...
                // Update payment status in DB
                await storage.updatePaymentReferenceStatus(paymentIntentFailed.id, 'rejected', paymentIntentFailed.last_payment_error?.message || 'failed', paymentIntentFailed.id);
                break;
            case 'payment_intent.processing':
                const paymentIntentProcessing = event.data.object;
                console.log('⏳ PaymentIntent processing:', paymentIntentProcessing.id);
                // Update payment status in DB
                await storage.updatePaymentReferenceStatus(paymentIntentProcessing.id, 'pending', 'processing', paymentIntentProcessing.id);
                break;
            default:
