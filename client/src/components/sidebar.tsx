...
export function Sidebar() {
  ...
  const navItems = [
    { href: "/provider-dashboard", icon: Home, label: "Início" },
    { href: "/agenda", icon: Calendar, label: "Agenda" },
    { href: "/messages", icon: MessageCircle, label: "Chat" },
    { href: "/provider-orders", icon: ClipboardList, label: "Pedidos" },
    { href: "/provider-settings", icon: Settings, label: "Configurações" },
  ];
      ...
      <nav className="flex-1 px-4 space-y-2">
        {navItems.map((item) => (
          <Link key={item.href} href={item.href}>
            ...
            >
              <item.icon className={`mr-3 h-5 w-5 ${isActiveRoute(item.href) ? "text-primary" : ""}`} />
              {item.label}
            </Button>
          </Link>
        ))}
